...
def main():
    fields = {
        ...
        "file_path": {"default": False, "type": "str"},
        "api_command": {"default": True, "type": "str"}
    }
    ...
    params = dict(
        ...
        container_volume="%s:/ansible/playbooks:ro" % (
    ...
    params[
        'pyaem_init'] = "aem = pyaem2.PyAem2('%(aem_username)s', '%(aem_password)s', '%(host)s', '%(port)s'); " % params
    ...
    params[
        'pyaem_error'] = "print( json.dumps({ 'failed': True, 'msg': result.message }) if result.is_failure() else json.dumps({ 'msg': result.message }) )"
    ...
    if any(True for match in
        ...
        params[
            'pyaem_cmd'] = "result = aem.%(api_command)s('%(group_name)s', '%(package_name)s', '%(package_version)s' ); " % params
        ...
        params[
            'pyaem_cmd'] = "result = aem.upload_package_sync('%(group_name)s', '%(package_name)s', '%(package_version)s', '/ansible/playbooks', force = 'true'); " % params
    else:
        module.fail_json(msg={
    ...
    params[
        'pyaemscriptmanual'] = "docker run -it --privileged --entrypoint %(container_entrypoint)s -v %(container_volume)s %(container_image)s %(pyaemscript)s" % params
